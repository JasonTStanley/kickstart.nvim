  ...
  for line in f:lines() do
    ...
    if line:match '^%s*\\documentclass' then
      ...
      line = line:gsub('\\input{([^}]+)}', function(path)
        if path:sub(1, 1) ~= '/' then path = root_dir .. '/' .. path end
        return '\\input{' .. path .. '}'
      end)
      lines[#lines + 1] = line
    end
...
local function get_outermost_block_from_lines(lines, lnum)
  ...
  for i = lnum, 1, -1 do
    ...
    if end_env then table.insert(skip_stack, end_env) end
  ...
  for i = outer_start + 1, total do
    local line = lines[i]
    if line:match(pat_begin) then depth = depth + 1 end
    if line:match(pat_end) then
      depth = depth - 1
      if depth == 0 then outer_end = i; break end
    end
  end
  ...
  local block_lines = {}
  for i = outer_start, outer_end do block_lines[#block_lines + 1] = lines[i] end
...
local function get_ref_label_at_cursor()
  ...
  while s <= #line do
    ...
    if ref_preview_cmds[cmd] and col >= cs and col <= ce then return label end
    s = ce + 1
  end
  return nil
end
...
local function preview_label_async(state, label, bufnr)
  ...
  for i, line in ipairs(lines) do
    if line:find(pattern, 1, true) then
      ...
      if content and content ~= '' then render_async(state, content, bufnr) end
      return
    end
  end
  ...
  vim.fn.jobstart(
    { 'rg', '-F', '--with-filename', '--line-number', '--no-heading',
      pattern, '--glob', '*.tex', dir },
    {
        ...
        vim.schedule(function()
          for _, result in ipairs(data) do
            ...
            if file and lnum_str then
              ...
              if f then
                local file_lines = {}
                for fl in f:lines() do file_lines[#file_lines + 1] = fl end
                f:close()
...
render_async = function(state, content, bufnr)
  ...
  state.render_job = vim.fn.jobstart({
    'sh', '-c',
    string.format(
      'cd %s && pdflatex -interaction=nonstopmode -halt-on-error preview.tex > /dev/null'
        .. ' && pdftoppm -r 400 -png -singlefile preview.pdf %s',
    ...
    on_exit = function(_, code)
      state.render_job = nil
      vim.schedule(function()
        if my_gen ~= state.render_generation then return end
        if code == 0 then
...
function M.attach(bufnr)
  ...
  vim.api.nvim_create_autocmd('CursorHold', {
    ...
    group  = aug,
    callback = function()
      if not state.auto_enabled then return end
  ...
  vim.api.nvim_create_autocmd('BufLeave', {
    buffer = bufnr,
    group  = aug,
    callback = function() close_panel(state) end,
  })
  ...
  vim.keymap.set('n', '<leader>lp', function()
    ...
    vim.notify(
      'LaTeX auto-preview ' .. (state.auto_enabled and 'enabled' or 'disabled'),
      vim.log.levels.INFO
    )
